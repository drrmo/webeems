...
const GeotaggingSchema = new Schema({
  _id: {
    type: String,
    validate: {
      validator(v) {
...
GeotaggingSchema.pre('save', async function(next) {
  ...
  if (!this._id) {
    ...
    try {
      ...
      const nextIndex = lastDoc.reduce((a, b) => Math.max(Number(b.ID.substring(8)) + 1, a),
        1);
    ...
    try {
      // eslint-disable-next-line no-underscore-dangle
      assert.strictEqual(Number(this._id.substring(8)) > 0, true);
      if (!this.ID) {
        this.ID = this._id;
      }
      if (!this.HouseID) {
...
GeotaggingSchema.set('toJSON', { getters: true }); // Enable toJSON getters

const Geotagging = model('Geotagging', GeotaggingSchema);
