...
var _sessionToken = require("../controllers/sessionToken");
var _Evacuation = _interopRequireDefault(require("../models/Evacuation"));
...
function getRandomUsername({
  ...
  return (0, _uniqueNamesGenerator.uniqueNamesGenerator)({
    dictionaries: [_uniqueNamesGenerator.animals, _uniqueNamesGenerator.adjectives, _uniqueNamesGenerator.colors, _uniqueNamesGenerator.names],
...
function getRandomOccupation({
  dictionary = ['Student', 'Customer Support Representative', 'Administrative Assistant', 'Content Writer', 'Sales Representative', 'Marketing Coordinator', 'IT Support Specialist', 'Account Manager', 'Receptionist', 'Data Entry Operator', 'Social Media Specialist', 'Project Coordinator', 'Event Coordinator', 'Research Assistant', 'Virtual Assistant', 'Technical Support Engineer', 'Quality Assurance Tester', 'Business Analyst', 'Community Manager', 'SEO Specialist', 'Online Tutor'],
  additionalDictionary = [],
  style = 'capital',
  seed = Date.now()
}) {
  const dictionaries = [...dictionary, ...additionalDictionary];
  const rand = (0, _seedrandom.default)(seed);
  return mapDictionaryRandom(dictionaries, style, rand);
}
function mapDictionaryRandom(dictionaries, style, rand) {
  // eslint-disable-next-line no-nested-ternary
  return [dictionaries[Math.floor(rand() * dictionaries.length)]].map(v => style === 'capital' ? v.split(' ').map(val => `${val[0].toUpperCase()}${val.substring(1).toLowerCase()}`).join(' ')
  // eslint-disable-next-line no-nested-ternary
  : style === 'upperCase' ? v.toUpperCase() : style === 'lowerCase' ? v.toLowerCase() : v).join();
}
function getRandom({
  dictionary = [_uniqueNamesGenerator.names],
  style,
  seed = Date.now()
}) {
  const rand = (0, _seedrandom.default)(seed);
  const dictionaries = dictionary.reduce((a, b) => b instanceof Array ? [...a, ...b] : b instanceof Object ? [...a, b] : [...a, b.toString()], []);
  // eslint-disable-next-line no-nested-ternary
  return mapDictionaryRandom(dictionaries, style, rand);
}
function getRandomContact() {
  return `+639${Math.floor(10 ** 8 + Math.random() * 9 * 10 ** 8)}`;
}
...
router.get('/', _sessionToken.createDefaultAdmin, (req, res) => {
  res.json({
    "message": "Default Admin has been created."
  }).end();
});
router.get('/add/geotagging', _sessionToken.verifiedAdmin, async (req, res, next) => {
  async function getResult() {
    ...
    for (let i = 0; i < memberCount; i++) {
      memberNames.push({
        ...
        Occ: getRandomOccupation({
          seed: getRandomCount(5000, Date.now())
        }),
    ...
    const Household = {
      Head: {
        ...
        Age: headAge,
        Occ: getRandomOccupation({
          seed: getRandomCount(4300, Date.now())
        }),
        ConNum: getRandomContact()
      },
      Members: memberNames
    ...
    try {
      return await _models.Geotagging.create({
        Household,
        ...
        BuildType: ['duplex', 'single', 'apartment', 'squatters', 'hotel'][getRandomCount(5)],
...
router.get('/add/user', _sessionToken.verifiedAdmin, async (req, res, next) => {
  const cnt = req.query.count ? Math.min(500, Math.max(1, Number(req.query.count))) : 1;
  const results = [];
  let currentCount;
  try {
    currentCount = await _models.User.count().exec();
    if (currentCount === 0 || (await _models.User.findOne({
      Username: 'admin'
    }).count().exec()) === 0) {
      results.push(await _models.User.create({
        Username: 'admin',
        Password_hash: await (0, _userController.getPasswordHash)('password'),
        Role: 'admin',
        ...
        Suffx: req.query.suffx || getRandom({
          dictionary: ['', 'Jr.', 'Sr.', 'I', '', '']
        }),
  ...
  async function getResult() {
    try {
      const adminCount = await _models.User.find({
        Role: 'admin'
      }).count();
      const AddedBy = (await _models.User.find({
        Role: 'admin'
      }))[getRandomCount(adminCount)].id;
      ...
      return await _models.User.create({
        ...
        Position: getRandom({
          dictionary: ['Barangay Captain', 'Barangay Kagawad', 'Purok Leader', 'Barangay SK Chairman', 'Barangay SK Kagawad', 'IT Support'],
          style: 'capital',
          seed: getRandomCount(2523, Date.now())
        }),
        Barangay: getRandom({
        ...
        Permissions: ['pre-evacuation-create', 'pre-evacuation-modify', 'pre-evacuation-delete', 'evacuation-modify', 'post-evacuation-modify', 'post-evacuation-print', 'pre-evacuation-scan', 'evacuation-scan', 'post-evacuation-scan'].shuffle().reduce((aa, bb, ii) => ii < permissionCount ? [...aa, bb] : aa, ['view-info']),
        AddedBy
      });
    } catch (error) {
      return getResult();
...
router.get('/add/evacuation', _sessionToken.verifiedAdmin, async (req, res, next) => {
  let assigns;
  let createdBy;
  try {
    createdBy = await _models.User.findOne({
      Role: 'admin'
    }).select('ID').exec();
    assigns = await _models.User.find().exec();
  } catch (error) {
  ...
  try {
    const created = await _Evacuation.default.create(randomData);
    const retrieved = await _Evacuation.default.findById(created.ID).populate('Assigned').populate('PreEvacuation.Evacuated.ID').populate('PreEvacuation.Evacuated.OperatedBy').populate('PostEvacuation.Evacuated.ID').populate('PostEvacuation.Evacuated.OperatedBy').exec();
    res.json({
      created,
      retrieved
    }).end();
...
router.post('/test/evacuation', _sessionToken.verifiedAdmin, async (req, res, next) => {
  const {
    evacid,
    proc,
    operator,
  ...
  try {
    const success = await _Evacuation.default.findById(evacid).exec();
    if (proc === 'pre-evacuation' || proc === 'post-evacuation' || proc === 'evacuation') {
      const array = updates.shift().split('.').reduce((a, b) => a === null ? success[b] : a[b], null);
